...
func (cac *CreateAddressCommand) Execute(ctx context.Context, city, country, number, street, postalCode, cologne string) (*domain.Address, error) {
	if city == "" || country == "" || number == "" || street == "" || postalCode == "" || cologne == "" {
		return nil, errors.New("todos los campos son requeridos")
	}

	address, err := domain.NewAddress(city, country, number, street, postalCode, cologne)
